...
public class BtService {
    ...
    private synchronized void setState(int state) {
        ...
        switch (mState) {
            case STATE_LISTEN:
                EventBusHelper.postSafely(new PrintMsgEvent(PrinterMsgType.MESSAGE_STATE_CHANGE, "等待连接"));
                break;
            case STATE_CONNECTING:
                EventBusHelper.postSafely(new PrintMsgEvent(PrinterMsgType.MESSAGE_STATE_CHANGE, "正在连接"));
                break;
            case STATE_CONNECTED:
                EventBusHelper.postSafely(new PrintMsgEvent(PrinterMsgType.MESSAGE_STATE_CHANGE, "已连接"));
                break;
            default:
                EventBusHelper.postSafely(new PrintMsgEvent(PrinterMsgType.MESSAGE_STATE_CHANGE, "未连接"));
                break;
        }
    ...
    public synchronized void connect(BluetoothDevice device) {
        ...
        EventBusHelper.postSafely(new PrintMsgEvent(PrinterMsgType.MESSAGE_TOAST, "正在连接蓝牙设备"));
    ...
    public synchronized void connected(BluetoothSocket socket,
        ...
        EventBusHelper.postSafely(new PrintMsgEvent(PrinterMsgType.MESSAGE_TOAST, "蓝牙设备连接成功"));
...
//         Send a failure message back to the Activity
        EventBusHelper.postSafely(new PrintMsgEvent(PrinterMsgType.MESSAGE_TOAST, "蓝牙连接失败,请重启打印机再试"));
        setState(STATE_NONE);
...
//         Send a failure message back to the Activity
        EventBusHelper.postSafely(new PrintMsgEvent(PrinterMsgType.MESSAGE_TOAST, "蓝牙连接断开"));
        setState(STATE_NONE);
        // Start the service over to restart listening mode
        BtService.this.start();
    }
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    // send data
                    EventBusHelper.postSafely(new BtMsgReadEvent(bytes, buffer));
                } catch (IOException e) {
